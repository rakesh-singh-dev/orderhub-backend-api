// src/services/parsers/nykaaParser.js

const BaseParser = require("./baseParser");

class NykaaParser extends BaseParser {
  constructor() {
    super("nykaa");
  }
  ...
  canParse(email) {
    ...
    return (
      sender.includes("nykaa") ||
      sender.includes("nykaa.com") ||
      sender.includes("@nykaa") ||
      sender.includes("noreply@nykaa") ||
      sender.includes("orders@nykaa") ||
      subject.includes("nykaa") ||
      subject.includes("order confirmation") ||
      subject.includes("order placed") ||
      subject.includes("your order") ||
      subject.includes("order shipped") ||
      subject.includes("order delivered")
    );
  }

  /**
   * Parse Nykaa email and extract order information
   */
  parse(email) {
  ...
  extractItems(html, text) {
    ...
    // Nykaa-specific item patterns (beauty/cosmetics focused)
    const itemPatterns = [
      /product[:\-]?\s*([^₹\n]+?)\s*₹\s*(\d+(?:,\d+)*(?:\.\d{2})?)/gi,
      /item[:\-]?\s*([^₹\n]+?)\s*₹\s*(\d+(?:,\d+)*(?:\.\d{2})?)/gi,
      /([^₹\n]+?)\s*₹\s*(\d+(?:,\d+)*(?:\.\d{2})?)/gi,
